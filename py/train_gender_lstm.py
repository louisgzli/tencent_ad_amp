...
def train(model, task, y_list, x_list, checkpoint_dir, checkpoint_prefix, device, batch_size=512, max_seq_len=100,
          lr=1e-3, resume_surfix=None, logger=None):
    """
    ...
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', factor=0.5, patience=1,
                                                           threshold=1e-5, threshold_mode='abs')
    if resume_surfix is not None:
        ...
        optimizer_artifact_path = os.path.join(checkpoint_dir,
                                               '{}_{}_opti.pth'.format(checkpoint_prefix, resume_surfix))
    ...
    # Main Loop
    dl_train = data_loader_v2(wv, y_list, x_list, input_split_path, None, batch_size=batch_size, shuffle=True,
                              train=True)
    dl_val = data_loader_v2(wv, y_list, x_list, input_split_path, None, batch_size=batch_size, shuffle=True,
                            train=False)
    for epoch, file_idx_list in task:
        ...
        while True:
            ...
            if logger:
                logger.info('Epoch {}/{} - iteration {} Done - Train Loss: {:.6f}, Learning Rate {:.7f}'.format(epoch,
                                                                                                                task[
                                                                                                                    -1][
                                                                                                                    0],
                                                                                                                iteration,
        ...
        if logger:
            logger.info('Epoch {}/{} - Updated Learning Rate: {:.8f}'.format(epoch, task[-1][0],
                                                                             optimizer.param_groups[0]['lr']))
        if logger:
            logger.info('Epoch {}/{} Done - Test Loss: {:.6f}, Test Accuracy: {:.6f}'.format(epoch, task[-1][0],
                                                                                             test_running_loss / test_n_batch,
                                                                                             acc_score))
...
if __name__ == '__main__':
    ...
    if len(sys.argv) == 5:
        ...
        if resume_file == 1:
            ...
            task = [(resume_epoch, np.arange(resume_file, 10))] + [(i, np.arange(1, 10)) for i in
                                                                   range(resume_epoch + 1, end_epoch + 1)]
